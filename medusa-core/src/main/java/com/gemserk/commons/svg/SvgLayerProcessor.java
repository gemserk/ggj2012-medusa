package com.gemserk.commons.svg;
...
public class SvgLayerProcessor {
	...
	// protected Stack<SvgGroup> groupStack;
	...
	public void process(Document document) {
		...
		svgParser.addHandler(new SvgInkscapeGroupHandler() {
			...
			protected void handle(SvgParser svgParser, SvgInkscapeGroup svgInkscapeGroup, Element element) {
				...
				// groupStack.push(svgInkscapeGroup);

				handleGroup(svgParser, svgInkscapeGroup, element);
			}
			...
			protected void postHandle(SvgParser svgParser, SvgInkscapeGroup svgInkscapeGroup, Element element) {
				...
				postHandleGroup(svgInkscapeGroup);
				transformStack.pop();
		...
		svgParser.addHandler(new SvgInkscapePathHandler() {
			...
			protected void handle(SvgParser svgParser, SvgPath svgPath, Element element) {
				...
				for (int i = 0; i < points.length; i++) {
					...
					point.set(tmp.x, svgDocument.getHeight() - tmp.y);
			...
			protected void postHandle(SvgParser svgParser, SvgPath svgPath, Element element) {
				postHandlePathObject(svgPath, element, vertices);
			}
	...
	protected void handleGroup(SvgParser svgParser, SvgInkscapeGroup svgInkscapeGroup, Element element) {

	}

	protected void postHandleGroup(SvgInkscapeGroup svgInkscapeGroup) {
	...
	protected void postHandlePathObject(SvgPath svgPath, Element element, Vector2[] vertices) {
